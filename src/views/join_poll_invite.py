...
from src.database.scripts import join_poll_invite
...
def get_join_poll_invites_for_my_group(group_id: int,
    ...
    user_id = credentials.subject["id"]
    try:
        return [schemas.JoinPollInvite.model_validate(i) for i in
                join_poll_invite.for_group.execute(db, user_id, group_id)]
