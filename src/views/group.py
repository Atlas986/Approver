...
def get_user_groups(db: Session = Depends(get_session),
                    credentials: JwtAuthorizationCredentials = Security(jwt_config.access_security)):
    user_id = credentials.subject["id"]
...
def get_all_users_of_group(group_id: int,
    ...
    user_id = credentials.subject["id"]
    try:
        return [schemas.USER_GROUP_relationship.model_validate(i) for i in
                db_group.get_members.execute(db, user_id=user_id, group_id=group_id)]
...
def my_relationship_with_group(group_id: int,
    ...
    user_id = credentials.subject["id"]
    try:
        return schemas.USER_GROUP_relationship.model_validate(
            db_group.get_user_relationship.execute(db, user_id, group_id))
