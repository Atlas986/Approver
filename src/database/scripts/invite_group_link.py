...
from src.database.scripts.utils import get_by, get_user_group_relationship, safe_get_invite_link_by_id
...
class by_user:
    def execute(db: Session, user_id: int) -> list[outer_models.Invite_group_link]:
        return [outer_models.Invite_group_link.model_validate(i) for i in
                get_by(db, models.Invite_group_link, models.Invite_group_link.created_by_id, user_id)]
...
class for_group:
    ...
    def execute(db: Session, user_id: int, group_id: int) -> list[outer_models.Invite_group_link]:
        ...
        if not models.GroupRoles.can_watch_all_invite_links(relationship.role):
            raise create.forbidden
        return [outer_models.Invite_group_link.model_validate(i) for i in
                get_by(db, models.Invite_group_link, models.Invite_group_link.group_id, group_id)]
...
class delete_by_id:
    ...
    def execute(db: Session, user_id: int, link_id: str):
        try:
            try:
                link: models.Invite_group_link = \
                    get_by(db, models.Invite_group_link, models.Invite_group_link.id, link_id)[0]
            except Exception:
                raise delete_by_id.link_not_found
            relationship = get_user_group_relationship(db, user_id, link.group_id)
            if (relationship is not None and outer_models.GroupRoles.can_delete_invite_link(
                    relationship.role)) or link.created_by_id == user_id:
                db.delete(link)
            else:
...
class use:
    ...
    def execute(db: Session, user_id: int, link_id: str):
        try:
            ...
            relationship: models.GROUP_USERS = db.query(models.GROUP_USERS).filter(
                models.GROUP_USERS.group_id == link.group_id, models.GROUP_USERS.user_id == user_id).first()
            if relationship is not None:
                raise use.already_in_group
            relationship = models.GROUP_USERS(group_id=link.group_id, user_id=user.id, added_by_id=link.created_by_id,
                                              role=link.role)
            db.add(relationship)
