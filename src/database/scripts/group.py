...
class create:
    ...
    def execute(db: Session, name: str, user_id: int) -> outer_models.Group:
        try:
            ...
            user_group: models.GROUP_USERS = models.GROUP_USERS(group_id=group.id, user_id=user.id, added_by_id=user.id,
                                                                role=models.GroupRoles.owner)
            db.add(user_group)
            db.commit()
...
class get_members:
    ...
    def execute(db: Session, user_id: int, group_id: int) -> list[outer_models.USER_GROUP_relationship]:
        ...
        if not outer_models.GroupRoles.can_watch_users(got_rights=relationship.role):
            raise get_members.forbidden
        return [outer_models.USER_GROUP_relationship.model_validate(i) for i in
                get_by(db, models.GROUP_USERS, models.GROUP_USERS.group_id, group_id)]
...
class get_for_user:
    ...
    def execute(db: Session, user_id: int) -> list[outer_models.Group]:
        return [outer_models.Group.model_validate(get_by(db, models.Group, models.Group.id, i.group_id)[0]) for i in
                get_by(db, models.GROUP_USERS, models.GROUP_USERS.user_id, user_id)]
