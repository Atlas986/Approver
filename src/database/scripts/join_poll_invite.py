...
from src.database import (outer_models,
                          models,
                          exceptions)
from src.database.scripts.utils import (get_by,
                                        get_user_group_relationship,
                                        safe_get_poll_by_id,
                                        get_poll_group_relationship)
...
class create:
    ...
    @staticmethod
    def execute(db: Session, created_by_id: int, poll_id: int, for_whom_id: int, role: models.PollRoles):
        try:
            try:
...
class for_group:
    ...
    def execute(db: Session, user_id: int, group_id: int) -> list[outer_models.Join_poll_invite]:
        ...
        if not models.GroupRoles.can_watch_join_poll_invites(relationship.role):
            raise for_group.forbidden
        return [outer_models.Join_poll_invite.model_validate(i) for i in
                get_by(db, models.Join_poll_invite, models.Join_poll_invite.for_whom_id, group_id)]
...
class accept:
    ...
    def execute(db: Session, user_id: int, invite_id: int):
        try:
            try:
                invite: models.Join_poll_invite = \
                    get_by(db, models.Join_poll_invite, models.Join_poll_invite.id, invite_id)[0]
            except Exception:
            ...
            if not models.GroupRoles.can_accept_join_poll_invites(relationship.role):
